...
const mostBlogs = (blogs) => {
  if (blogs.length > 0) {
    ...
    const biggestNum = _.maxBy(
      Object.entries(sumsOfBLogs),
      ([author, blogs]) => blogs,
    );
    const result = {
      author: biggestNum[0],
...
const mostLikes = (blogs) => {
  if (blogs.length > 0) {
    const authors = _.groupBy(blogs, "author");
    const sumOfLikes = _.mapValues(authors, (blogs) =>
      _.sumBy(blogs, (blog) => blog.likes),
    );
    const biggestLike = _.maxBy(
      Object.entries(sumOfLikes),
      ([author, likes]) => likes,
    );
    const result = {
